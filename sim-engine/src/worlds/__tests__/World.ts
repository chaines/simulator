...
describe('Basic World Class', () => {
  ...
  beforeEach(() => {
    ...
    agent1 = new BasicAgent([1, 1]);
    agent2 = new BasicAgent([1, 2]);
  });
  ...
  describe('getNearby', () => {
    ...
    beforeEach(() => {
      agent3 = new BasicAgent([10, 12]);
      agent4 = new BasicAgent([15, 12]);
      agent5 = new BasicAgent([100, 90]);
      world.addEntity(agent1);
      world.addEntity(agent2);
    ...
    it('Should be empty with a range of 0', () => {
      const results = world.getNearby([15, 15], 0);
      expect(results.length).toBe(0);
    });

    it('Should detect nearby entities along the x axis', () => {
      let results = world.getNearby([12, 12], 2);
      expect(results.length).toBe(1);
      results = world.getNearby([12, 12], 3);
      expect(results.length).toBe(2);
    });

    it('Should detect nearby entities along the y axis', () => {
      let results = world.getNearby([10, 11], 1);
      expect(results.length).toBe(1);
      results = world.getNearby([10, 10], 2);
      expect(results.length).toBe(1);
    });

    it('Should detect all entities when radius encompasses whole board', () => {
      let results = world.getNearby([50, 50], Math.sqrt(50000));
  ...
  describe('inBounds', () => {
    it('Should return true for all integer values in the constraints', () => {
      for (let i = 0; i < 100; i++) {
        for (let j = 0; j < 100; j++) {
          expect(world.inBounds([i, j])).toBe(true);
        }
      }
    });
    it('should return false if either x or y is out of bounds in either direction', () => {
      expect(world.inBounds([50, 101])).toBe(false);
      expect(world.inBounds([101, 50])).toBe(false);
      expect(world.inBounds([-1, 50])).toBe(false);
      expect(world.inBounds([50, -1])).toBe(false);
    });
    it('should properly adjust to the size of the world', () => {
      expect(world.inBounds([99, 99])).toBe(true);
      expect(new BasicWorld(50, 50).inBounds([99, 99])).toBe(false);
    });
  });
});
